...
((archiveCustomerAuth) => {
    archiveCustomerAuth.customerAuth = async (call, metaData) => {
        try {
            var query = mysqlHelper.format(`select * from customer_loggedin_sessions where id = ?`, [call.id]);
            ...
            console.error(metaData, `Error at archiveCustomerAuth.customerAuth`, error);
            return false;
        }
    }
