(() => {
    ...
    module.exports = async (req, res, next) => {
        ...
        const payload = requestHelper(req, res, cbs.merchant_service.CreateAuthTypeRequest)
        ...
        try {
            client.createAuthType(payload, meta, grpcClientHelper.setTimeOut(), (err, response) => {
                if (err) {
                    return responseHelper.sendErrorResponse(err, res, req.metaData);
                }

                return responseHelper.sendSuccessResponse(response.status, response, cbs.merchant_service.CreateAuthTypeResponse
                    , res);
            });
        } catch (err) {
            console.error(req.metaData, '[ADMIN: CREATE AUTH TYPE]', err);
            return responseHelper.sendErrorResponse(err, res, req.metaData);
        }
    }
})();